function [ z_out, empty_tag, MPC_time ] = Central_write( f, g, rel_mat)
...
%   此处显示详细说明
    ...
    H = zeros(n*Np, n*Np);
    F = zeros(n*Np, 1);
    A = zeros(n*Np+m*2*Np, n*Np);
    B = zeros(n*Np+m*2*Np, 1);
    empty_tag = 0;
    for i = 1:n
        H((i-1)*Np+1:i*Np, (i-1)*Np+1:i*Np) = f{1,i}.H_o;
        F((i-1)*Np+1:i*Np, 1) = f{1,i}.f_o;
        A((i-1)*Np+1:i*Np, (i-1)*Np+1:i*Np) = f{1,i}.A_o;
...
%     Z = Other_boundFun(Z, [Lb,Ub]);
    file = fopen('central.txt','w');
    for j = 1:n*Np
        for k = 1:n*Np
            fprintf(file, '%20.4f', H(j,k));
        end
    end
    fprintf(file, '\n');
    ...
    for j = 1:n*Np+m*2*Np
        for k = 1:n*Np
            fprintf(file, '%20.4f', A(j, k));
        end
    end
    fprintf(file, '\n');
    
    for j = 1:n*Np+m*2*Np
        fprintf(file, '%20.4f', B(j));
    end
    fprintf(file, '\n');
    
    for j = 1:n*Np
       fprintf(file, '%20.4f', Lb(j));
    end
    fprintf(file, '\n');
    
    for j = 1:n*Np
        fprintf(file, '%20.4f', Ub(j));
    end     
    fprintf(file, '\n');
    fclose(file);
