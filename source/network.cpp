...
double central_cal(string central_file, int M, int W, int m, real_t *cen_ans)
{
	int c_H = M*m;
	int c_A = M * m + (W - M) * 2 * m;
...
bool ADMM::check_conv_record(int tag, int max_step, int output_step)
{
	float Ax = 0, Bz = 0, Alambda = 0, pri_val = 0;
	R_2 = 0, S_2 = 0;
#pragma omp parallel for
...
#pragma omp parallel for
	...
	eps_dual = (float)(sqrt(n) * eps_abs + sqrt(Alambda) * eps_rel), eps_dual = pow(eps_dual, 2);
	string out_string;
	string file_name = "converge_syn.txt";
	ofstream out(file_name, iostream::app);
	out_string = to_string(tag) + " " + to_string(sqrt(R_2)) + " " + to_string(sqrt(eps_pri)) + " " + to_string(sqrt(S_2)) + " " + to_string(sqrt(eps_dual)) + "\n";
	out << out_string;
	out.close();
	if (tag % output_step == 0)
	{
...
real_t * ADMM::getSolution(real_t *cen_ans, double cen_time)
{
	float max_error = 0.0;
	...
	// rho & parallel time & central time & first error & all error & step 
	string file_name = "vehnum" + to_string(M) + ".txt";
	string out_string;
	ofstream out(file_name, iostream::app);
	out_string = to_string((int)rho) + " " + to_string(total_time) + " " + to_string(cen_time) \
		+ " " + to_string(first_error * 100) + " " + to_string(max_error * 100) + " " + to_string(iter_step) + "\n";
	out << out_string;
	out.close();
...
double ADMM::getTotalTIme()
{
	return total_time;
}
// ******************** the solution fashion you can edit ***************************

/* real parallel fashion; fixed compute point;
This function will save a txt named as "vehnum5.txt", where 5 is the veh num can be substitute;
it will remains rho & total_time & cen_time & first erro & max error & iteration step */
...
// real asynchornous, fixed point, and no stop cretia instead with max step to terminal
...
/* pesudo parallel asynchronous fashion; adaptive compute point;
This function will save a txt named as "vehnum5.txt", where 5 is the veh num can be substitute;
it will remains rho & total_time & cen_time & first erro & max error & iteration step */
void ADMM::AsyFor(int max_step, int output_step, int thread_num, int rand_num)
{
	clock_t start, end;
	omp_set_num_threads(thread_num);
	int tag = 0;
...
/* pesudo parallel fashion; adaptive compute point;
This function will save a txt named as "vehnum5.txt", where 5 is the veh num can be substitute;
it will remains rho & total_time & cen_time & first erro & max error & iteration step */
void ADMM::synFor(int max_step, int output_step, int thread_num, int rand_num)
{
	...
	do {
		...
		updateDual();
		t2 = clock();
		synTime += (double)(t2 - t1) / CLOCKS_PER_SEC;
		updatePriFor(tag, rand_num);
		updateRho();
		tag += 1;
	} while (check_conv(tag, max_step, output_step));
...
This function will save a txt named as "vehnum5.txt", where 5 is the veh num can be substitute;
it will remains rho & total_time & cen_time & first erro & max error & iteration step 
if rand_num == w, the algorithm is sy; else if rand_num == 1, the algorithm is asy;
it wiill generate a txt named as "convg_sy.txt", where is the algorithm model, can be modified in function check_conv_record*/
void ADMM::Iteration(int max_step, int output_step, int thread_num, int rand_num)
{
	clock_t start, end;
	omp_set_num_threads(thread_num);
	int tag = 0;
	...
	do {
		...
		updateRho();
		tag += 1;
	} while (check_conv_record(tag, max_step, output_step));
	end = clock();
	double endtime = (double)(end - start) / CLOCKS_PER_SEC;
	iter_step = tag;
	printf("The total time is %f, the iteration number is %d\n", total_time, tag);
